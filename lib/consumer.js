...
module.exports = (STREAM_NAME, GROUP_NAME, CONSUMER_ID, connectionArgs) => {
    ...
    const consume = async (PROCESSOR, DIRECTION = '>', BLOCK = false) => {
        ...
        if (entry) {
            const [[STREAM, [[MSGID, CONTENT]]]] = entry;

            // DO something with the MSG

            debug('Processing...', { STREAM, DIRECTION, MSGID, CONTENT });
            await PROCESSOR(CONTENT.reduce((acc, curr, i, arr) => {
                if(i%2 === 0){
                    acc[curr] = JSON.parse(arr[i+1]);
                } return acc;
            },{}));
